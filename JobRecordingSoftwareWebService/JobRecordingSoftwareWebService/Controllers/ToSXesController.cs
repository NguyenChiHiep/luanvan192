...
namespace JobRecordingSoftwareWebService.Controllers
{
    ...
    */
    public class ToSXesController : ODataController
    {
        ...
        // GET: odata/ToSXes(5)/NguyenCongs
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs([FromODataUri] int key)
        {
            return db.ToSXes.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs);
        }

        // GET: odata/ToSXes(5)/NguyenCongs1
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs1([FromODataUri] int key)
        {
            return db.ToSXes.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs1);
        }
