...
namespace JobRecordingSoftwareWebService.Controllers
{
    ...
    */
    public class NhanViensController : ODataController
    {
        ...
        // GET: odata/NhanViens(5)/NguyenCongs
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs([FromODataUri] int key)
        {
            return db.NhanViens.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs);
        }

        // GET: odata/NhanViens(5)/NguyenCongs1
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs1([FromODataUri] int key)
        {
            return db.NhanViens.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs1);
        }

        // GET: odata/NhanViens(5)/NguyenCongs2
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs2([FromODataUri] int key)
        {
            return db.NhanViens.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs2);
        }

        // GET: odata/NhanViens(5)/ThongKeCongViecTrongNgays
        [EnableQuery]
        public IQueryable<ThongKeCongViecTrongNgay> GetThongKeCongViecTrongNgays([FromODataUri] int key)
        {
            return db.NhanViens.Where(m => m.Id == key).SelectMany(m => m.ThongKeCongViecTrongNgays);
        }

        // GET: odata/NhanViens(5)/ToSX
        [EnableQuery]
        public SingleResult<ToSX> GetToSX([FromODataUri] int key)
        {
            return SingleResult.Create(db.NhanViens.Where(m => m.Id == key).Select(m => m.ToSX));
        }
