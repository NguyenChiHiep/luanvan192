...
namespace JobRecordingSoftwareWebService.Controllers
{
    ...
    */
    public class MaysController : ODataController
    {
        ...
        // GET: odata/Mays(5)/NguyenCongs
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs([FromODataUri] int key)
        {
            return db.Mays.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs);
        }

        // GET: odata/Mays(5)/ToSX
        [EnableQuery]
        public SingleResult<ToSX> GetToSX([FromODataUri] int key)
        {
            return SingleResult.Create(db.Mays.Where(m => m.Id == key).Select(m => m.ToSX));
        }
