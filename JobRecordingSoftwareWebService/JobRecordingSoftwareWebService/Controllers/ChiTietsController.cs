...
namespace JobRecordingSoftwareWebService.Controllers
{
    ...
    */
    public class ChiTietsController : ODataController
    {
        ...
        // GET: odata/ChiTiets(5)/LenhSanXuat
        [EnableQuery]
        public SingleResult<LenhSanXuat> GetLenhSanXuat([FromODataUri] int key)
        {
            return SingleResult.Create(db.ChiTiets.Where(m => m.Id == key).Select(m => m.LenhSanXuat));
        }
        ...
        // GET: odata/ChiTiets(5)/NguyenCongs
        [EnableQuery]
        public IQueryable<NguyenCong> GetNguyenCongs([FromODataUri] int key)
        {
            return db.ChiTiets.Where(m => m.Id == key).SelectMany(m => m.NguyenCongs);
        }
